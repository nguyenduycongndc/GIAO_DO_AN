...
export class OTPScreen extends Component {
  ...
  setToken = async token => {
    ...
    NavigationUtil.navigate(SCREEN_ROUTER_AUTH.REGISTER_INFO, {
      token: token,
  ...
  startCountdown = () => {
    this.inter = setInterval(() => {
      if (parseInt(this.state.timeCountDown) <= 0) clearInterval(this.inter);
      else this.setState({ timeCountDown: this.state.timeCountDown - 1 });
    }, 1000);
  };
  ...
  requestConfirmOTP(code) {
    ...
    let payload = {
      Phone: this.state.phone,
      OTP: code || this.state.code
    };
    CallApiHelper(
      API.requestConfirmOTP,
      payload,
      this,
      res => {
      ...
      <ScreenComponent
        ...
        renderView={
          <>
            ...
            <CodeInput
              ref={this.field}
              normalizeCode={this.handleChangeText}
              variant="border-b"
              autoFocus={true}
              codeLength={6}
            ...
            {!this.state.timeCountDown > 0 ? (
              <TouchableOpacity
                ...
                children={
                  <>
                    <FastImage
                      source={R.images.ic_resend}
                      style={styles.ic_resend}
                    />
                    <Text
              ...
              <Text
                ...
                children={this.state.timeCountDown}
              />
            )}
          </>
        }
